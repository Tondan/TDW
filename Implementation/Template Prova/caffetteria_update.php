...
echo $piattodelgiorno,"<br>\n";*/
    ...
    if(strcmp($target_file,$dir)==0){
        ...
        $oid = mysql_query("UPDATE tdw.caffetteria SET Titolo = '$nome', Descrizione = '$descrizione', Img = '$target_file' WHERE ID_Caffetteria = '$key';");
                                    }
    else{
        $oid = mysql_query(" UPDATE tdw.caffetteria SET Titolo =  '$nome', Descrizione =  '$descrizione', Img ='$target_file' WHERE ID_Caffetteria = '$key';");
        if($oid){
...
if($oid){
    ...
    header("refresh:2; caffetteria_manager.php");
} else{
