...
echo $piattodelgiorno,"<br>\n";*/
    ...
    if(strcmp($target_file,$dir)==0){
        ...
        $oid = mysql_query("UPDATE tdw.presentazione_reservation SET Titolo = '$nome', Descrizione = '$descrizione', Img = '$target_file' WHERE Titolo = '$key';");
                                    }
    else{
        $oid = mysql_query("UPDATE tdw.presentazione_reservation SET Titolo = '$nome', Descrizione = '$descrizione', Img = '$target_file' WHERE Titolo = '$key';");
        if($oid){
...
if($oid){
    ...
    header("refresh:2; reservation_manager.php");
} else{
